...
if (args['decoder'] == "Mem2Seq"):
    if args['dataset']=='kvr':
        ...
        from utils.utils_wiki_mem2seq import * 
    elif args['dataset']=='nmt':
        from utils.utils_NMT import *                
    else: 
...
cnt_1 = 0   
### LOAD DATA
train, dev, test, lang, max_len, max_r = prepare_data_seq(args['task'],batch_size=int(args['batch']),shuffle=True)
...
for epoch in range(1000):
    ...
    if((epoch+1) % int(args['evalp']) == 0):    
        acc = model.evaluate(epoch, dev,avg_best, BLEU)    
