...
#                        print('ind==',ind)
                        ind_str = str(ind).replace("tensor(", "").replace(")", "")
 #                       print(self.lang.index2word[int(ind_str)])
                        temp.append(self.lang.index2word[int(ind_str)])
                    from_which.append('v')
    ...
    def evaluate(self,epoch, dev,avg_best,BLEU=False):

        logging.info("STARTING EVALUATION")
        outf = open("./data/webnlg2/out/out"+str(epoch)+".txt",'w')
        acc_avg = 0.0
        wer_avg = 0.0
        bleu_avg = 0.0
        acc_P = 0.0
        ...
        outf.writelines(str(hyp)+"==="+str(ref)+"\n") 
        outf.writelines("bleu_score"+str(bleu_score)+"\n")         
